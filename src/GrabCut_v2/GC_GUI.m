...
%CurrRes = Image{CurrInd};
openImage(handles);


function openImage(handles)
    global im
...
global OutputData;
while ishghandle(hObject) % checks to see if figure still exists

    % short pause to allow gui controls update (loop runs too fast)
    pause(0.001);
    % constantly reassign the output until the figure is closed
...
if isempty(CurrRes)
    msgbox('No Mask Generated. Draw a polygon and then press run.')
else
    OutputData = im2bw(CurrRes, 0);
    close all;
end